...
class Mover():
    ...
    def replacement_bin_identifier(self):
        ...
        up in a unique bin that only contains water particles. If no bins are
        found containing solely water within reasonable time, the best alternative
        option is used.
    ...
    def position_generator_aa(self, chosen_bins, oxy_pos):
        ...
        by randomly picking a position and calculating the shortest distance between
        this position and the other particles in the bin. this process is repeated
        10.000 times. This function is used in all-atom simulations where the center
        of geometry of water molecules is used to relocate them
    ...
    def water_replacement_gro_aa(self, indices, new_pos, gro_file, oxy_pos):
        """
        new positions are assigned to water particles that have to be moved,
        subsequently a new gro file is created
        ...
        for i, e in enumerate(indices):
            if oxy_pos == 0:
                ...
                ox_water = self.all_atoms.select_atoms(istring).center_of_geometry()
            ...
            if oxy_pos == 1:
                ...
                cog_water = self.all_atoms.select_atoms(istring).center_of_geometry()
